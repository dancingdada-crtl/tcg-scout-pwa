...
export async function getReportPhotoForReport(reportId){if(!supabase)throw new Error('Supabase is not configured.');const {data,error}=await supabase.from('report_photos').select('*').eq('report_id',reportId).order('created_at',{ascending:false}).limit(1).maybeSingle();if(error)throw new Error(`Photo lookup failed: ${error.message}`);return data?{id:data.id,reportId:data.report_id,storeId:data.store_id,path:data.storage_path,bytes:data.bytes||0,width:data.width||null,height:data.height||null,mimeType:data.mime_type||'',createdAt:data.created_at,expiresAt:data.expires_at||null}:null;}
export async function getPreviousStoreReportPhoto(storeId,beforeCreatedAt,currentReportId){if(!supabase)throw new Error('Supabase is not configured.');let query=supabase.from('report_photos').select('*').eq('store_id',storeId).neq('report_id',currentReportId).order('created_at',{ascending:false}).limit(1);if(beforeCreatedAt)query=query.lt('created_at',beforeCreatedAt);const {data,error}=await query.maybeSingle();if(error)throw new Error(`Previous photo lookup failed: ${error.message}`);return data?{id:data.id,reportId:data.report_id,storeId:data.store_id,path:data.storage_path,bytes:data.bytes||0,width:data.width||null,height:data.height||null,mimeType:data.mime_type||'',createdAt:data.created_at,expiresAt:data.expires_at||null}:null;}


export async function loadTcgCategories(){
  if(!supabase) throw new Error('Supabase is not configured.');
  const {data,error}=await supabase.from('tcg_categories').select('*').order('name');
  if(error) throw error;
  return (data||[]).map(c=>({id:c.id,name:c.name,emoji:c.emoji||'',active:c.active!==false,createdBy:c.created_by||null}));
}

export async function createTcgCategory(c){const name=(c?.name||'').trim();if(!name)throw new Error('Add a category name');const {data:{user},error:ue}=await supabase.auth.getUser();if(ue)throw ue;if(!user)throw new Error('Sign in required');const {data:existing,error:se}=await supabase.from('tcg_categories').select('*').ilike('name',name).limit(1);if(se)throw se;if(existing?.length){const row=existing[0];const {data,error}=await supabase.from('tcg_categories').update({name,emoji:c.emoji||row.emoji||null,active:true,updated_at:new Date().toISOString()}).eq('id',row.id).select().single();if(error)throw error;return {id:data.id,name:data.name,emoji:data.emoji||'',active:data.active!==false};}const {data,error}=await supabase.from('tcg_categories').insert({name,emoji:c.emoji||null,created_by:user.id}).select().single();if(error)throw error;return {id:data.id,name:data.name,emoji:data.emoji||'',active:data.active!==false};}
...
export async function loadPersonalAlerts(){const {data,error}=await supabase.from('personal_alerts').select('*').order('created_at',{ascending:false});if(error)throw error;return (data||[]).map(a=>({id:a.id,title:a.title,storeId:a.store_id||null,weekdays:a.weekdays||[],timeOfDay:String(a.time_of_day||'09:00').slice(0,5),timeZone:a.time_zone||'UTC',active:a.active!==false,createdAt:a.created_at}))}
export async function createPersonalAlert(a){const {data:{user}}=await supabase.auth.getUser();if(!user)throw new Error('Sign in required');const {data,error}=await supabase.from('personal_alerts').insert({member_id:user.id,title:a.title,store_id:a.storeId||null,weekdays:a.weekdays,time_of_day:a.timeOfDay,time_zone:a.timeZone||'UTC',active:true}).select().single();if(error)throw error;return data}
export async function updatePersonalAlert(id,patch){const payload={};if('active'in patch)payload.active=!!patch.active;if(patch.title!=null)payload.title=patch.title;if(patch.weekdays)payload.weekdays=patch.weekdays;if(patch.timeOfDay)payload.time_of_day=patch.timeOfDay;if(patch.storeId!==undefined)payload.store_id=patch.storeId||null;payload.updated_at=new Date().toISOString();const {error}=await supabase.from('personal_alerts').update(payload).eq('id',id);if(error)throw error}
export async function deletePersonalAlert(id){const {error}=await supabase.from('personal_alerts').delete().eq('id',id);if(error)throw error}
