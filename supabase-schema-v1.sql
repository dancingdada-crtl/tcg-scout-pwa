-- TCG Scout starter schema for Supabase/Postgres.
-- Run only after creating a Supabase project and reviewing policies for your deployment.
create extension if not exists pgcrypto;

create type public.app_role as enum ('member','admin');
create type public.report_type as enum ('stocked','empty','possible','line','evidence');
...
-- Helper used by policies. SECURITY DEFINER avoids recursive policy evaluation.
create or replace function public.is_admin()
returns boolean language sql stable security definer set search_path=public as $$
  select exists(select 1 from public.profiles where id=auth.uid() and role='admin');
$$;

-- Authenticated group members can read core data.
...
-- Members can contribute and soft-manage stores/products.
...
-- Member activity is private to each user plus sole admin.
...
-- Deliberately no member DELETE policies. Permanent deletion should go through admin-only controls.
