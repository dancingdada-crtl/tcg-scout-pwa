...
let data=load();
let state={magicSent:false,inviteStep:false,inviteCode:'',view:'home',sheet:null,selectedStore:null,memberHistoryId:null,activityStore:null,activityMetric:'stock',mapSort:'nearest',userLocation:null,mapMoveEnabled:false,routeSelected:[],mapStatuses:['stock','empty','unsure','unchecked'],mapIndicatorFilters:[],manageKind:'stores',editId:null,analyticsMode:'when',analytics:{metric:'stock',tcg:'All',productId:'All',storeId:'All',period:'All',range:30,groupBy:'day'},adminLogAction:'All',adminLogEntity:'All',activityRange:7,activitySeenBeforeOpen:null,toast:null};
...
function recentReportsBlock(){const seen=state.activitySeenBeforeOpen||localStorage.getItem('chasedex-last-activity-seen');const reps=visibleReports().slice().sort((a,b)=>new Date(b.createdAt||b.occurredAt)-new Date(a.createdAt||a.occurredAt)).slice(0,3);return `<div class="section-head recent-head"><div><h2>Recent Reports</h2><div class="tiny">Newest community updates across all stores.</div></div></div><div class="card recent-reports-card">${reps.map(r=>{const s=data.stores.find(x=>x.id===r.storeId),p=data.products.find(x=>x.id===r.productId),isNew=!!seen&&new Date(r.createdAt||r.occurredAt)>new Date(seen)&&r.memberId!==currentMember()?.id;return `<button class="recent-report-row" data-action="report-detail" data-id="${r.id}"><span class="recent-status ${r.status==='stock'?'green':r.status==='empty'?'red':'orange'}"></span><span><b>${esc(s?.name||'Store')} · ${reportStatusLabel(r.status)}${isNew?' <em class="new-badge">NEW</em>':''}</b><small>${p?esc(p.name)+' · ':''}${relativeTime(r.createdAt||r.occurredAt)}</small><small class="reporter-inline">Reported by ${reporterText(r)}</small></span><span>›</span></button>`}).join('')||'<div class="empty">No reports yet.</div>'}</div>`}
function activity(){
 const stores=activeStores();if(!stores.length)return `<section class="section">${recentReportsBlock()}<div class="empty">Add a store to view activity.</div></section>`;
...
function reportDetailSheet(id){const r=data.reports.find(x=>x.id===id);if(!r)return'';const s=data.stores.find(x=>x.id===r.storeId),p=data.products.find(x=>x.id===r.productId);return `<div class="sheet flow-sheet"><div class="sheet-card"><div class="sheet-title"><div><h2>${reportStatusLabel(r.status)} ${reportFlags(r)}</h2><div class="tiny">${esc(s?.name||'Store')}</div></div><button class="close" data-action="close-sheet">×</button></div>${reporterCard(r)}<div class="detail-grid"><span>Event time</span><b>${fmtDate(r.occurredAt)} · ${r.period} · ${fmtTime(r.occurredAt)}</b><span>Submitted</span><b>${fmtDate(r.createdAt)} · ${fmtTime(r.createdAt)}</b><span>Product</span><b>${esc(p?.name||'—')}</b><span>Source</span><b>${esc(r.source||'—')}</b><span>Price</span><b>${r.price!=null?`$${Number(r.price).toFixed(2)} ${esc(r.condition||'')}`:'—'}</b></div>${r.notes?`<div class="note-box">${esc(r.notes)}</div>`:''}<div class="row"><button class="btn" data-action="edit-report" data-id="${r.id}">Edit report</button><button class="btn secondary" data-action="share-report" data-id="${r.id}">Share</button></div></div></div>`}
