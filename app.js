...
import { backendConfigured, getAuthSession, onAuthStateChange, sendMagicLink, sendInviteMagicLink, validateInviteCode, createInviteCode, revokeInviteCode, updateReleaseMessage, signInWithPassword, updatePassword, signOut as backendSignOut, recordLogin, recordSession, loadSharedData, createReport, createStore, createProduct, saveAnalytics as backendSaveAnalytics, uploadProfileImage, updateMyProfile, updateRankingTiers, setMemberEnabled, updateStore, deleteStore, updateProduct, deleteProduct, updateReport, deleteReport, saveIndicator, deleteIndicator, setReportFeedback, subscribeRealtime } from './backend.js';
...
function loginView(){
 ...
 const headline='Hunt Cards. Chase trends. Know what's next. Live local data and collector – powered insights from ChaseDex',access='Member signup is invite-only. Use your password for normal sign-in. Magic Link remains available for first-time access or recovery.';
...
function activity(){
 ...
 if(!state.activityStore||!stores.some(s=>s.id===state.activityStore))state.activityStore=stores[0].id;const storeId=state.activityStore;
 return `<section class="section"><div class="section-head"><div><h2>7-day activity</h2><div class="tiny">One store · all statuses and indicators stay in sync.</div></div></div><div class="card"><div class="field"><label>Store</label><select id="activity-store" data-change="activity-store">${stores.map(s=>`<option value="${s.id}" ${storeId===s.id?'selected':''}>${esc(s.name)}</option>`).join('')}</select></div><div class="chart-title">${esc(data.stores.find(s=>s.id===storeId)?.name)}</div>${sevenBars(storeId)}</div><div class="section-head"><h2>Reports</h2><span class="tiny">Tap to view / edit / delete</span></div>${visibleReports().filter(r=>r.storeId===storeId).sort((a,b)=>new Date(b.occurredAt)-new Date(a.occurredAt)).slice(0,50).map(activityRow).join('')||'<div class="empty">No activity yet.</div>'}</section>`}
...
function analyticsFilterBar(){const a=state.analytics,tcgs=[...new Set(activeProducts().map(p=>p.tcg))];return `<div class="card analytics-builder"><div class="two-col"><div class="field"><label>Range</label><select id="a-range" data-change="analytics"><option value="7" ${Number(a.range)===7?'selected':''}>7 days</option><option value="14" ${Number(a.range)===14?'selected':''}>14 days</option><option value="30" ${Number(a.range)===30?'selected':''}>30 days</option><option value="90" ${Number(a.range)===90?'selected':''}>90 days</option></select></div><div class="field"><label>Store</label><select id="a-store" data-change="analytics"><option value="All">All stores</option>${activeStores().map(x=>`<option value="${x.id}" ${a.storeId===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div></div><div class="two-col"><div class="field"><label>TCG</label><select id="a-tcg" data-change="analytics"><option>All</option>${tcgs.map(x=>`<option ${a.tcg===x?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="field"><label>Product</label><select id="a-product" data-change="analytics"><option value="All">All products</option>${activeProducts().filter(p=>a.tcg==='All'||p.tcg===a.tcg).map(p=>`<option value="${p.id}" ${a.productId===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></div></div></div>`}
function analyticsOverview(reps){const stock=reps.filter(r=>r.status==='stock').length,empty=reps.filter(r=>r.status==='empty').length,missed=reps.filter(r=>r.status==='unsure').length,checks=stock+empty+missed,rate=checks?Math.round(stock/checks*100):0,stores=new Set(reps.map(r=>r.storeId)).size,products=new Set(reps.filter(r=>r.productId).map(r=>r.productId)).size,conf=confidenceFor(reps);const days=Array.from({length:Math.min(14,Number(state.analytics.range)||30)},(_,i)=>{const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()-(Math.min(14,Number(state.analytics.range)||30)-1-i));return d});const max=Math.max(1,...days.map(d=>reps.filter(r=>sameDay(r.occurredAt,d)).length));return `<div class="analytics-kpis"><div><b>${reps.length}</b><small>Reports</small></div><div><b>${rate}%</b><small>Stock rate</small></div><div><b>${missed}</b><small>Missed stock</small></div><div><b>${stores}</b><small>Active stores</small></div><div><b>${products}</b><small>Drops reported</small></div><div><b>${conf.label}</b><small>${reps.length} observations · ${conf.score}% confidence</small></div></div><div class="card"><h3>Recent activity trend</h3><div class="spark-bars">${days.map(d=>{const n=reps.filter(r=>sameDay(r.occurredAt,d)).length;return `<span title="${fmtDate(d)} · ${n}"><i style="height:${Math.max(5,n/max*100)}%"></i><small>${d.toLocaleDateString(undefined,{weekday:'narrow'})}</small></span>`}).join('')}</div></div>${analyticsSignals(reps)}`}
function analyticsWhere(reps){const rows=activeStores().map(store=>{const rr=reps.filter(r=>r.storeId===store.id),stock=rr.filter(r=>r.status==='stock').length,missed=rr.filter(r=>r.status==='unsure').length,empty=rr.filter(r=>r.status==='empty').length,total=stock+missed+empty,rate=total?Math.round(stock/total*100):0,periods=data.settings.periods.map(p=>[p,rr.filter(r=>r.period===p&&r.status==='stock').length]).sort((a,b)=>b[1]-a[1]),conf=confidenceFor(rr);return{store,stock,missed,empty,total,rate,best:periods[0]?.[1]?periods[0][0]:'—',conf}}).filter(x=>x.total).sort((a,b)=>b.rate-a.rate);return `<div class="card"><h3>Store availability</h3><div class="analysis-table"><div class="analysis-row head"><span>Store</span><span>Stock</span><span>Missed</span><span>Best</span><span>Confidence</span></div>${rows.map(x=>`<div class="analysis-row"><span><b>${esc(x.store.name)}</b><small>${x.total} observations</small></span><span>${x.rate}%</span><span>${x.missed}</span><span>${x.best}</span><span>${x.conf.label}</span></div>`).join('')||'<div class="empty">No matching store observations.</div>'}</div></div>${analyticsSignals(reps)}`}
function analyticsWhen(reps){const weekdays=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],periods=data.settings.periods;const cells=weekdays.flatMap((d,di)=>periods.map(p=>reps.filter(r=>new Date(r.occurredAt).getDay()===di&&r.period===p&&r.status==='stock').length)),max=Math.max(1,...cells);return `<div class="card"><h3>Best observed windows</h3><div class="time-heat"><div></div>${periods.map(p=>`<b>${p.slice(0,3)}</b>`).join('')}${weekdays.map((d,di)=>`<b>${d}</b>${periods.map(p=>{const n=reps.filter(r=>new Date(r.occurredAt).getDay()===di&&r.period===p&&r.status==='stock').length;return `<i style="--heat:${n/max}" title="${d} ${p}: ${n} stock reports">${n||'·'}</i>`}).join('')}`).join('')}</div><div class="confidence-note">Confidence: ${confidenceFor(reps).label} · ${reps.length} observations</div></div>${analyticsSignals(reps)}`}
function analyticsDrops(reps){const groups=new Map();for(const r of reps){if(!r.productId)continue;const a=groups.get(r.productId)||[];a.push(r);groups.set(r.productId,a)}const rows=[...groups].map(([id,rr])=>{const p=data.products.find(x=>x.id===id),stock=rr.filter(r=>r.status==='stock').length,missed=rr.filter(r=>r.status==='unsure').length,total=rr.filter(r=>['stock','empty','unsure'].includes(r.status)).length,prices=rr.filter(r=>r.price!=null).map(r=>Number(r.price)),avg=prices.length?prices.reduce((a,b)=>a+b,0)/prices.length:null;return{p,rr,stock,missed,total,rate:total?Math.round(stock/total*100):0,avg,stores:new Set(rr.map(r=>r.storeId)).size,conf:confidenceFor(rr)}}).sort((a,b)=>b.rr.length-a.rr.length);return `<div class="card"><h3>Product / drop intelligence</h3>${rows.map(x=>`<div class="drop-analysis"><div><b>${esc(x.p?.name||'Product')}</b><small>${esc(x.p?.tcg||'')} · ${x.rr.length} sightings · ${x.stores} stores</small></div><div><b>${x.rate}% stock</b><small>${x.missed} missed${x.avg!=null?` · avg $${x.avg.toFixed(2)}`:''} · ${x.conf.label} confidence</small></div></div>`).join('')||'<div class="empty">No matching product reports.</div>'}</div>${analyticsSignals(reps)}`}
...
function analytics(){const mode=state.analyticsMode||'overview',reps=analyticsReports();const body=mode==='where'?analyticsWhere(reps):mode==='when'?analyticsWhen(reps):mode==='drops'?analyticsDrops(reps):analyticsOverview(reps);return `<section class="section"><div class="section-head"><div><h2>Analytics</h2><div class="tiny">What happened, where to go, when to check and what is dropping.</div></div></div><div class="analytics-modes">${[['overview','Overview'],['where','Where'],['when','When'],['drops','Drops']].map(([v,l])=>`<button class="chip ${mode===v?'active':''}" data-action="analytics-mode" data-mode="${v}">${l}</button>`).join('')}</div>${analyticsFilterBar()}${body}</section>`}
function products(){
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(a==='save-release-message'){await updateReleaseMessage(document.getElementById('admin-release-message').value.trim());await refreshShared();toast('Release message saved');return}
...
document.addEventListener('change',e=>{const t=e.target;if(t.dataset.change==='route-store'){toggleRouteStore(t.dataset.id);render();return}if(t.dataset.change==='activity-store'){state.activityStore=t.value;render()}if(t.dataset.change==='analytics'){state.analytics={...state.analytics,tcg:document.getElementById('a-tcg').value,productId:document.getElementById('a-product').value,storeId:document.getElementById('a-store').value,range:Number(document.getElementById('a-range').value)};if(t.id==='a-tcg')state.analytics.productId='All';render()}});
