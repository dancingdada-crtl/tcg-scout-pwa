...
function analytics(){
 const a=state.analytics,reps=filteredAnalytics(),groups=analyticsGroups(reps),max=Math.max(1,...groups.map(x=>x.n));
 const isPrice=a.metric==='price',chart=isPrice?'Line chart':'Bar chart',me=currentMember();
 const saved=me?data.savedFilters.filter(f=>f.ownerId===me.id&&f.metric):[];
 const metricOptions=[['stock','Stock reports'],['empty','No stock reports'],...activeIndicators().map(i=>[`indicator:${i.id}`,`${i.emoji} ${i.label}`]),['price','Average observed price']];
