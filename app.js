...
import { backendConfigured, getAuthSession, onAuthStateChange, sendMagicLink, sendInviteMagicLink, validateInviteCode, createInviteCode, revokeInviteCode, updateLoginSettings, signInWithPassword, updatePassword, signOut as backendSignOut, recordLogin, recordSession, loadSharedData, createReport, createStore, createProduct, saveAnalytics as backendSaveAnalytics, uploadProfileImage, updateMyProfile, updateRankingTiers, setMemberEnabled, updateStore, deleteStore, updateProduct, deleteProduct, updateReport, deleteReport, saveIndicator, deleteIndicator, setReportFeedback, subscribeRealtime } from './backend.js';
...
let data=load();
let state={magicSent:false,inviteStep:false,inviteCode:'',view:'home',sheet:null,selectedStore:null,activityStore:'All',activityMetric:'stock',mapSort:'nearest',userLocation:null,routeSelected:[],mapStatuses:['stock','empty','unchecked'],mapIndicatorFilters:[],manageKind:'stores',editId:null,analyticsMode:'availability',analytics:{metric:'stock',tcg:'All',productId:'All',storeId:'All',period:'All',range:30,groupBy:'day'},adminLogAction:'All',adminLogEntity:'All',toast:null};
...
function loginView(){
 ...
 const s=data.settings||{},headline=s.loginHeadline||'Fast local restock, price and activity intelligence for your group.',access=s.loginAccessMessage||'Member signup is invite-only. Use your password for normal sign-in. Magic Link remains available for first-time access or recovery.';
...
function nav(){const items=[['home','🗺️','Map'],['activity','▥','Activity'],['analytics','⌁','Analytics'],['products','◫','Manage']];if(isMember())items.push(['members','👥','Members']);if(currentMember()?.role==='admin')items.push(['admin','⚙','Admin']);const unread=unreadReports();return `<nav class="bottom-nav" style="grid-template-columns:repeat(${items.length},1fr)">${items.map(([v,i,l])=>`<button class="nav-btn ${state.view===v?'active':''}" data-action="nav" data-view="${v}"><span class="nav-icon">${i}${v==='activity'&&unread?`<b class="nav-badge">${unread>99?'99+':unread}</b>`:''}</span>${l}</button>`).join('')}</nav>`}

function updateNotice(){const s=data.settings||{};if(!authSession||!s.appVersion||s.appVersion===localStorage.getItem('chasedex-ack-version'))return'';return `<div class="magic-modal-backdrop"><div class="magic-modal"><h2>ChaseDex updated · v${esc(s.appVersion)}</h2><p>${esc(s.releaseMessage||'A new ChaseDex version is available.')}</p>${s.reinstallRequired?`<div class="magic-first-time"><b>Reinstall recommended</b><span>Android: uninstall ChaseDex → open the ChaseDex site in Chrome → menu → <strong>Add to Home screen / Install app</strong> → sign back in.</span></div>`:''}<button class="btn wide" data-action="ack-update">Got it</button></div></div>`}
function openSheet(name){state.sheet=name;history.pushState({chasedex:true,view:state.view,sheet:name},'',location.href);render()}
function shell(){const flow=['store','report','report-detail'].includes(state.sheet);if(flow)return `${sheet()}${updateNotice()}${state.toast?`<div class="toast">${esc(state.toast)}</div>`:''}`;return `<div class="shell">${topbar()}<main>${state.view==='home'?home():state.view==='activity'?activity():state.view==='analytics'?analytics():state.view==='products'?products():state.view==='members'?members():admin()}</main>${nav()}</div>${state.sheet?sheet():''}${updateNotice()}${state.toast?`<div class="toast">${esc(state.toast)}</div>`:''}`}
function home(){
 const stores=sortedHomeStores(),locNote=state.userLocation?`<div class="distance-note">Nearest first · straight-line distance. Driving distance and ETA may differ.</div>`:`<div class="distance-note">Tap Locate Me to sort nearest → farthest.</div>`;
 ...
 return `<section class="section"><div class="section-head"><div><h2>What's near me</h2><div class="tiny">Today's status, indicators and routes.</div></div><button class="pill" data-action="map-locate">◎ Locate Me</button></div>${locNote}<div class="status-toggle-row">${statusToggles}</div>${indicatorToggles?`<div class="indicator-filter-row">${indicatorToggles}</div>`:''}<div class="map-wrap"><div id="map"></div></div></section><section class="section"><div class="section-head"><h2>Stores</h2><span class="tiny">Nearest → farthest</span></div>${stores.map(store=>{const r=todayLatestReport(store.id),st=mapStatus(store.id),dist=storeDistance(store),selected=selectedRouteStore(store.id);const detail=r?`${r.status==='stock'?'Stock':'No stock'} · ${r.period} · ${fmtTime(r.occurredAt)} ${reportFlags(r)}`:'Hasn\'t been checked today';return `<div class="store-list-card ${selected?'route-selected':''}"><button class="store-main" data-action="map-focus-store" data-id="${store.id}"><span class="legend-dot ${st.cls}"></span><span><b>${esc(store.name)} ${storeTodayIndicatorIds(store.id).map(id=>indicatorById(id)?.emoji||'').join(' ')}</b><small>${detail}${dist!=null?` · ${dist.toFixed(dist<10?1:0)} mi`:''}</small></span></button><label class="route-check" title="Add to route"><input type="checkbox" data-change="route-store" data-id="${store.id}" ${selected?'checked':''}><span>Route</span></label></div>`}).join('')||'<div class="empty">No stores match these filters.</div>'}</section>${routeBar}`;
}
function renderMap(){
 ...
 const map=L.map(el,{zoomControl:true}).setView(center,state.userLocation?13:11);window._map=map;window._storeMarkers=new Map();
 L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);
 const pts=[];
 valid.forEach(s=>{const st=mapStatus(s.id),emojis=storeTodayIndicatorIds(s.id).map(id=>indicatorById(id)?.emoji||'').join('');const icon=L.divIcon({className:'chasedex-marker-wrap',html:`<div class="map-pin ${st.cls}"><span class="map-pin-dot"></span>${emojis?`<span class="pin-emoji">${emojis}</span>`:''}</div>`,iconSize:[38,46],iconAnchor:[19,42]});const marker=L.marker([Number(s.lat),Number(s.lng)],{icon,zIndexOffset:100}).addTo(map).bindTooltip(esc(s.name));marker.on('click',()=>{state.selectedStore=s.id;openSheet('store')});window._storeMarkers.set(s.id,marker);pts.push([Number(s.lat),Number(s.lng)])});
...
function metricLabel(m){
 if(m==='stock')return 'Stock reports';
 if(m==='empty')return 'No stock reports';
 if(m==='price')return 'Average observed price';
 if(m?.startsWith('indicator:')){const i=indicatorById(m.slice(10));return i?`${i.emoji} ${i.label}`:'Indicator'}
 return m||'Reports';
}
function filteredAnalytics(){
 const a=state.analytics,cutoff=new Date();cutoff.setDate(cutoff.getDate()-Number(a.range||30));
 return visibleReports().filter(r=>{const p=data.products.find(x=>x.id===r.productId);return new Date(r.occurredAt)>=cutoff&&(a.tcg==='All'||p?.tcg===a.tcg)&&(a.productId==='All'||r.productId===a.productId)&&(a.storeId==='All'||r.storeId===a.storeId)&&(a.period==='All'||r.period===a.period)&&(a.metric==='price'?r.price!=null:metricMatch(r,a.metric))});
}
function analyticsGroups(reps){
 const a=state.analytics;let labels=[];
 if(a.groupBy==='period')labels=['Morning','Noon','Afternoon','Evening'];
 else if(a.groupBy==='weekday')labels=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
 else labels=last7().map(d=>d.toLocaleDateString('en-US',{month:'numeric',day:'numeric'}));
 return labels.map(label=>{const matched=reps.filter(r=>a.groupBy==='period'?r.period===label:a.groupBy==='weekday'?new Date(r.occurredAt).toLocaleDateString('en-US',{weekday:'short'})===label:new Date(r.occurredAt).toLocaleDateString('en-US',{month:'numeric',day:'numeric'})===label);const n=a.metric==='price'?(matched.length?matched.reduce((sum,r)=>sum+Number(r.price||0),0)/matched.length:0):matched.length;return{label,n}});
}
function lineChart(groups){
 const w=320,h=150,pad=22,max=Math.max(1,...groups.map(g=>g.n));
 const pts=groups.map((g,i)=>{const x=pad+(groups.length===1?0:i*(w-pad*2)/(groups.length-1));const y=h-pad-(g.n/max)*(h-pad*2);return{x,y,g}});
 return `<div class="line-chart"><svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Price trend"><polyline points="${pts.map(p=>`${p.x},${p.y}`).join(' ')}" fill="none" stroke="currentColor" stroke-width="3"/>${pts.map(p=>`<circle cx="${p.x}" cy="${p.y}" r="4" fill="currentColor"/>`).join('')}</svg><div class="line-labels">${groups.map(g=>`<span><b>${g.n?`$${g.n.toFixed(0)}`:'—'}</b><small>${esc(g.label)}</small></span>`).join('')}</div></div>`;
}
function analytics(){
 const a=state.analytics,mode=state.analyticsMode||'availability',me=currentMember();
 const modeMetrics={availability:[['stock','Stock reports'],['empty','No stock reports']],indicators:activeIndicators().map(i=>[`indicator:${i.id}`,`${i.emoji} ${i.label}`]),products:[['price','Average observed price'],['stock','Product sightings']]};
 const options=modeMetrics[mode]||modeMetrics.availability;if(!options.some(x=>x[0]===a.metric))a.metric=options[0]?.[0]||'stock';
 const reps=filteredAnalytics(),groups=analyticsGroups(reps),max=Math.max(1,...groups.map(x=>x.n)),isPrice=a.metric==='price';
 const graph=isPrice?lineChart(groups):`<div class="analytics-bars">${groups.map(g=>`<div class="analytics-bar-row"><span>${esc(g.label)}</span><div class="track"><i style="width:${(g.n/max)*100}%"></i></div><b>${g.n}</b></div>`).join('')}</div>`;
 const stock=visibleReports().filter(r=>r.status==='stock').length,empty=visibleReports().filter(r=>r.status==='empty').length,total=stock+empty,rate=total?Math.round(stock/total*100):0;
 const summary=mode==='availability'?`<div class="analytics-summary"><div><b>${rate}%</b><small>stock rate</small></div><div><b>${total}</b><small>checks</small></div><div><b>${activeStores().filter(x=>mapStatusKey(x.id)==='unchecked').length}</b><small>unchecked today</small></div></div>`:mode==='indicators'?`<div class="recommend">Choose any Admin-created indicator, then compare it by store, day, weekday or time bucket.</div>`:`<div class="recommend">Use Product + Price to see sightings, average observed price, store and condition patterns.</div>`;
 return `<section class="section"><div class="section-head"><div><h2>Analytics</h2><div class="tiny">Pick a question first, then narrow it with filters.</div></div></div><div class="analytics-modes">${[['availability','Availability'],['indicators','Indicators'],['products','Products / Price']].map(([v,l])=>`<button class="chip ${mode===v?'active':''}" data-action="analytics-mode" data-mode="${v}">${l}</button>`).join('')}</div>${summary}<div class="card analytics-builder"><div class="field"><label>Analyze</label><select id="a-metric" data-change="analytics">${options.map(([v,l])=>`<option value="${v}" ${a.metric===v?'selected':''}>${esc(l)}</option>`).join('')}</select></div><div class="two-col"><div class="field"><label>Store</label><select id="a-store" data-change="analytics"><option value="All">All stores</option>${activeStores().map(x=>`<option value="${x.id}" ${a.storeId===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div><div class="field"><label>Range</label><select id="a-range" data-change="analytics">${[7,30,90,365].map(x=>`<option value="${x}" ${Number(a.range)===x?'selected':''}>${x} days</option>`).join('')}</select></div></div><div class="two-col"><div class="field"><label>TCG</label><select id="a-tcg" data-change="analytics"><option>All</option>${[...new Set(activeProducts().map(p=>p.tcg))].map(x=>`<option ${a.tcg===x?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="field"><label>Product</label><select id="a-product" data-change="analytics"><option value="All">All products</option>${activeProducts().filter(p=>a.tcg==='All'||p.tcg===a.tcg).map(p=>`<option value="${p.id}" ${a.productId===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></div></div><div class="two-col"><div class="field"><label>Time</label><select id="a-period" data-change="analytics"><option>All</option>${data.settings.periods.map(x=>`<option ${a.period===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>Compare by</label><select id="a-group" data-change="analytics"><option value="day" ${a.groupBy==='day'?'selected':''}>Recent day</option><option value="weekday" ${a.groupBy==='weekday'?'selected':''}>Day of week</option><option value="period" ${a.groupBy==='period'?'selected':''}>Time bucket</option></select></div></div></div><div class="card"><div class="chart-title">${esc(metricLabel(a.metric))} · ${reps.length} matching reports</div>${graph}${me?'<button class="btn secondary wide" data-action="save-analytics">Save this view</button>':''}</div></section>`;
}
function products(){
...
function members(){const me=currentMember();if(!me)return '<div class="empty">Sign in as a member to view the member list.</div>';const ranked=data.members.slice().filter(m=>m.enabled!==false).sort((a,b)=>pointsFor(b)-pointsFor(a));const mine=(data.invites||[]).filter(i=>i.createdBy===me.id&&!i.redeemedAt&&!i.revokedAt);return `<section class="section"><div class="section-head"><div><h2>Members</h2><div class="tiny">Contribution rankings and private invites.</div></div><button class="pill" data-action="edit-profile">Edit my profile</button></div><div class="card ranking-list">${ranked.map((m,i)=>`<div class="ranking-row"><div class="rank-num">${i+1}</div>${avatar(m)}<div><b>${esc(m.name)}</b><small>${esc(rankTitle(m))} · ${pointsFor(m)} points</small></div><div class="rank-stats"><b>${m.reports||0}</b><small>reports</small></div></div>`).join('')}</div><div class="card"><div class="section-head compact"><div><h3>Invite someone</h3><div class="tiny">Single-use code · expires in 7 days.</div></div></div><div class="field"><label>Invitee label <span class="optional">optional</span></label><input id="invite-label" placeholder="Mike, local group, etc."></div><button class="btn wide" data-action="create-invite">Generate invite code</button>${mine.length?`<div class="invite-list">${mine.map(i=>`<div class="invite-row"><span><b>${esc(i.code)}</b><small>${esc(i.label||'Unlabeled')} · expires ${fmtDate(i.expiresAt)}</small></span><button class="chip" data-action="copy-invite" data-code="${esc(i.code)}">Copy</button></div>`).join('')}</div>`:''}</div><div class="tiny contribution-note">Contribution score: 1 point per report + 1 point per confirmation.</div></section>`}
function admin(){
 if(currentMember()?.role!=='admin')return'<div class="empty">Admin only.</div>';
 const action=state.adminLogAction||'All',entity=state.adminLogEntity||'All';const filtered=(data.changeLog||[]).filter(l=>(action==='All'||String(l.action).toLowerCase()===({Created:'create',Updated:'update',Deleted:'delete'}[action]||action).toLowerCase())&&(entity==='All'||String(l.entity_type).toLowerCase()===entity.toLowerCase())).slice(0,20);const invites=data.invites||[];const s=data.settings||{};
 return `<section class="section"><div class="section-head"><div><h2>Admin</h2><div class="tiny">Indicators, invites, login copy and audit.</div></div><button class="pill" data-action="export-excel">Export Excel</button></div><div class="card"><h3>Login messages</h3><div class="field"><label>Login heading message</label><textarea id="admin-login-headline">${esc(s.loginHeadline||'')}</textarea></div><div class="field"><label>Login access message</label><textarea id="admin-login-access">${esc(s.loginAccessMessage||'')}</textarea></div><div class="field"><label>Release / reinstall message</label><textarea id="admin-release-message">${esc(s.releaseMessage||'')}</textarea></div><button class="btn wide" data-action="save-login-settings">Save login & release messages</button></div><div class="card"><div class="section-head compact"><div><h3>Report indicators</h3><div class="tiny">Choices appear in Quick Report, Map and Activity.</div></div><button class="chip" data-action="indicator-add">＋ Add</button></div>${(data.indicators||[]).map(i=>`<div class="indicator-admin-row"><span class="indicator-emoji">${esc(i.emoji)}</span><span><b>${esc(i.label)}</b><small>${i.active?'Active':'Removed'}</small></span><button class="chip" data-action="indicator-edit" data-id="${i.id}">Edit</button></div>`).join('')}</div><div class="card"><h3>Invite history</h3>${invites.slice(0,20).map(i=>`<div class="change-row"><b>${esc(i.code)} · ${i.redeemedAt?'Redeemed':i.revokedAt?'Revoked':'Active'}</b><small>${esc(i.creatorName||'Member')} → ${esc(i.redeemedEmail||i.label||'Not redeemed')}</small><small>Created ${fmtDate(i.createdAt)}${i.redeemedAt?` · used ${fmtDate(i.redeemedAt)}`:''}</small>${!i.redeemedAt&&!i.revokedAt?`<button class="chip" data-action="revoke-invite" data-id="${i.id}">Revoke</button>`:''}</div>`).join('')||'<div class="empty">No invites yet.</div>'}</div><div class="card"><div class="section-head compact"><div><h3>Change log</h3><div class="tiny">Latest 20 matching records. Export contains the full loaded audit history.</div></div></div><div class="filter-row">${['All','Created','Updated','Deleted'].map(x=>`<button class="chip ${action===x?'active':''}" data-action="admin-log-action" data-value="${x}">${x}</button>`).join('')}</div><div class="filter-row">${['All','Store','Product','Report','Indicator'].map(x=>`<button class="chip ${entity===x?'active':''}" data-action="admin-log-entity" data-value="${x}">${x}</button>`).join('')}</div>${filtered.map(l=>`<div class="change-row"><b>${esc(l.action)} · ${esc(l.entity_type)}</b><small>${esc(l.actor_name||l.actor_id||'Unknown')} · ${fmtDate(l.changed_at)} ${fmtTime(l.changed_at)}</small></div>`).join('')||'<div class="empty">No matching changes.</div>'}</div></section>`;
}
...
function reportSheet(storeId){const now=periodFor(new Date()),recent=[...activeProducts()].sort((a,b)=>{const ar=visibleReports().filter(r=>r.productId===a.id).at(0)?.occurredAt||'',br=visibleReports().filter(r=>r.productId===b.id).at(0)?.occurredAt||'';return br.localeCompare(ar)});return `<div class="sheet flow-sheet"><div class="sheet-card quick-report"><div class="sheet-title"><div><h2>Quick report</h2><div class="tiny">Fast selections first; optional details stay visible.</div></div><button class="close" data-action="close-sheet">×</button></div><div class="field"><label>Store</label><select id="r-store">${activeStores().map(s=>`<option value="${s.id}" ${storeId===s.id?'selected':''}>${esc(s.name)}</option>`).join('')}</select></div><div class="field"><label>Status</label><div class="radio-grid">${radio('r-status','stock','Stock',true)}${radio('r-status','empty','No stock',false)}</div></div><div class="field"><label>Time bucket</label><div class="radio-grid four">${data.settings.periods.map(x=>radio('r-period',x,x,now===x)).join('')}</div></div>${activeIndicators().length?`<div class="field"><label>Indicators <span class="optional">optional</span></label><div class="indicator-grid">${activeIndicators().map(i=>`<label class="indicator-choice"><input class="r-indicator" data-id="${i.id}" type="checkbox"><span><b>${esc(i.emoji)}</b>${esc(i.label)}</span></label>`).join('')}</div></div>`:''}<div class="field"><label>Product <span class="optional">optional</span></label><input id="r-product-search" list="product-options" placeholder="Search or type product"><datalist id="product-options">${recent.map(p=>`<option value="${esc(p.tcg+' — '+p.name)}" data-id="${p.id}"></option>`).join('')}</datalist><input id="r-product" type="hidden"><div class="product-quick-picks">${recent.slice(0,4).map(p=>`<button type="button" class="chip" data-action="pick-product" data-id="${p.id}">${esc(p.name)}</button>`).join('')}</div></div><div class="two-col compact-fields"><div class="field"><label>Price</label><input id="r-price" inputmode="decimal" placeholder="$ 0.00"></div><div class="field"><label>Condition</label><select id="r-condition"><option value="">—</option><option>Sealed</option><option>Unsealed</option><option>Damaged</option></select></div></div><div class="field"><label>Source</label><select id="r-source">${['Firsthand','Friend','Phone call','Social media','Store employee','Other'].map(x=>`<option>${x}</option>`).join('')}</select></div><div class="two-col compact-fields"><div class="field"><label>Approx. event time</label><input id="r-occurred" type="datetime-local"></div><div class="field"><label>Notes</label><input id="r-notes" placeholder="Optional note"></div></div><button class="btn wide" data-action="submit-report">Submit quick report</button></div></div>`}
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(a==='recovery-magic'){const email=document.getElementById('email')?.value.trim();if(!email){loginNotice='Enter your existing member email first.';render();return}await sendMagicLink(email);pendingEmail=email;state.magicSent=true;render();return}
  if(a==='invite-magic-login'){const code=document.getElementById('invite-code')?.value.trim().toUpperCase(),email=document.getElementById('invite-email-login')?.value.trim();if(!code||!email){loginNotice='Enter both invite code and email.';render();return}await validateInviteCode(code);await sendInviteMagicLink(email,code);pendingEmail=email;state.inviteCode=code;state.magicSent=true;loginNotice='';render();return}
  ...
  if(a==='pick-product'){const p=data.products.find(x=>x.id===el.dataset.id);const hidden=document.getElementById('r-product'),search=document.getElementById('r-product-search');if(p&&hidden&&search){hidden.value=p.id;search.value=`${p.tcg} — ${p.name}`;document.querySelectorAll('.product-quick-picks .chip').forEach(x=>x.classList.toggle('active',x.dataset.id===p.id))}return}
  ...
  if(a==='save-login-settings'){await updateLoginSettings(document.getElementById('admin-login-headline').value.trim(),document.getElementById('admin-login-access').value.trim(),document.getElementById('admin-release-message').value.trim());await refreshShared();toast('Login settings saved');return}
...
document.addEventListener('change',e=>{const t=e.target;if(t.dataset.change==='route-store'){toggleRouteStore(t.dataset.id);render();return}if(t.dataset.change==='activity-store'){state.activityStore=t.value;render()}if(t.dataset.change==='analytics'){state.analytics={metric:document.getElementById('a-metric').value,tcg:document.getElementById('a-tcg').value,productId:document.getElementById('a-product').value,storeId:document.getElementById('a-store').value,period:document.getElementById('a-period').value,range:Number(document.getElementById('a-range').value),groupBy:document.getElementById('a-group').value};render()}});
...
document.addEventListener('input',e=>{
 ...
 if(e.target.id==='r-product-search'){const val=e.target.value.trim().toLowerCase(),p=activeProducts().find(x=>`${x.tcg} — ${x.name}`.toLowerCase()===val||x.name.toLowerCase()===val);const h=document.getElementById('r-product');if(h)h.value=p?.id||'';return}
...
async function init(){
 try{
  ...
  data=await loadSharedData(authSession);data.indicators??=[];data.changeLog??=[];data.invites??=[];save();applyBranding();appReady=true;render();if(authSession){requestCurrentLocation().then(loc=>{state.userLocation=loc;state.mapSort='nearest';render()}).catch(()=>{})}
