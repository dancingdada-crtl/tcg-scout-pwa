...
import { backendConfigured, getAuthSession, onAuthStateChange, sendMagicLink, signInWithPassword, updatePassword, signOut as backendSignOut, recordLogin, recordSession, loadSharedData, createReport, createStore, setStoreArchived, createProduct, setProductArchived, saveAnalytics as backendSaveAnalytics, uploadProfileImage, updateMyProfile, uploadBrandIcon, updateBranding, updateRankingTiers, setMemberEnabled, adminUpdateStore, adminUpdateProduct, adminUpdateReport, setReportFeedback, subscribeRealtime } from './backend.js';
...
let data=load();
let state={view:'home',sheet:null,selectedStore:null,activityStore:'All',activityMetric:'stock',adminDataType:'reports',adminEditId:null,analytics:{metric:'stock',tcg:'All',productId:'All',storeId:'All',period:'All',range:30,groupBy:'day'},toast:null};
...
function products(){
 ...
 const addStore=member?`<div class="card manage-card"><div class="section-head compact"><div><h3>Add store</h3><div class="tiny">Search first, verify, then save.</div></div></div>${geoapifyReady()?`<div class="field store-lookup"><label>Find store or address</label><input id="store-search" autocomplete="off" placeholder="Try Target Lowell MA"><div id="store-suggestions" class="lookup-results" hidden></div><div class="tiny">Selecting a result fills the address and coordinates.</div></div>`:`<div class="lookup-setup">Automatic lookup needs your Geoapify key. Manual entry still works.</div>`}<div class="field"><label>Name</label><input id="new-store" placeholder="Store name"></div><div class="field"><label>Address</label><input id="new-address" placeholder="Street, city, state"></div><div class="two-col"><div class="field"><label>Latitude</label><input id="new-lat" inputmode="decimal" placeholder="Auto-filled"></div><div class="field"><label>Longitude</label><input id="new-lng" inputmode="decimal" placeholder="Auto-filled"></div></div><div id="store-preview-wrap" class="store-preview-wrap" hidden><div class="tiny preview-label">Location preview</div><div id="store-preview-map"></div></div><button class="btn wide" data-action="add-store">Add store</button><div class="tiny" style="margin-top:8px">The preview stays compact so the form remains easy to use on mobile.</div></div>`:'';
 const stores=member?`<div class="card manage-card"><div class="section-head compact"><div><h3>Stores</h3><div class="tiny">${data.stores.length} total</div></div></div>${data.stores.map(s=>`<div class="trend-row"><div><b>${esc(s.name)}</b><div class="tiny">${esc(s.address||'No address')} · ${s.active?'Active':'Archived'}</div></div><button class="chip" data-action="toggle-store" data-id="${s.id}">${s.active?'Archive':'Restore'}</button></div>`).join('')||'<div class="empty">No stores yet.</div>'}</div>`:'';
 return `<section class="section"><div class="section-head"><div><h2>Manage</h2><div class="tiny">${member?'Add stores and products, then manage what the group tracks.':'Browse tracked products.'}</div></div>${member?`<button class="pill" data-action="open-product-add">＋ Product</button>`:''}</div>${addStore}${stores}<div class="card sku-card"><div class="field"><label>SKU / UPC web lookup</label><div class="inline-input"><input id="sku-lookup" inputmode="numeric" placeholder="Enter SKU or UPC"><button class="btn" data-action="sku-lookup">Search web</button></div><div class="tiny">Opens a web search in a new tab. No paid product API is used.</div></div></div><div class="section-head compact"><h3>Products</h3><span class="tiny">${activeProducts().length} active</span></div>${activeProducts().map(p=>{const reps=data.reports.filter(r=>r.productId===p.id&&r.price!=null).sort((a,b)=>new Date(b.occurredAt)-new Date(a.occurredAt));const latest=reps[0];return `<button class="product-card" data-action="product-detail" data-id="${p.id}"><div><b>${esc(p.name)}</b><small>${esc(p.tcg)}${p.sku?` · SKU ${esc(p.sku)}`:''}</small></div><div class="product-price">${latest?`$${Number(latest.price).toFixed(2)}<small>${esc(latest.condition||'Observed')}</small>`:'No price'}</div></button>`}).join('')||'<div class="empty">No active products.</div>'}</section>`}
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(a==='select-store-result'){
   ...
   if(name&&!name.value.trim())name.value=storeResultName(r);if(address)address.value=storeResultAddress(r);if(lat)lat.value=r.lat??'';if(lng)lng.value=r.lon??'';if(search)search.value=storeResultName(r);box.hidden=true;renderStorePreview(r.lat,r.lon);toast('Location selected');return;
  }
  if(a==='add-store'){
