...
import { backendConfigured, getAuthSession, onAuthStateChange, sendMagicLink, signInWithPassword, updatePassword, signOut as backendSignOut, recordLogin, recordSession, loadSharedData, createReport, createStore, setStoreArchived, createProduct, setProductArchived, saveAnalytics as backendSaveAnalytics, uploadProfileImage, updateMyProfile, updateRankingTiers, setMemberEnabled, adminUpdateStore, adminUpdateProduct, adminUpdateReport, setReportFeedback, subscribeRealtime } from './backend.js';
...
let data=load();
let state={view:'home',sheet:null,selectedStore:null,activityStore:'All',activityMetric:'stock',adminDataType:'reports',adminEditId:null,mapSort:'recent',userLocation:null,routeMode:false,routeSelected:[],analytics:{metric:'stock',tcg:'All',productId:'All',storeId:'All',period:'All',range:30,groupBy:'day'},toast:null};
...
function mapStatus(id){const r=latestReport(id);if(!r)return statusMeta.unchecked;if(r.status==='stock')return statusMeta.stock;if(r.status==='empty')return statusMeta.empty;return statusMeta.unchecked}
function mapFlags(r){return r?.flags?.line?'👥':''}
...
function sortedHomeStores(){const stores=activeStores().slice();if(state.mapSort==='nearest'&&state.userLocation)return stores.sort((a,b)=>(storeDistance(a)??Infinity)-(storeDistance(b)??Infinity));return stores.sort((a,b)=>new Date(latestReport(b.id)?.occurredAt||0)-new Date(latestReport(a.id)?.occurredAt||0))}
...
function home(){
 ...
 const locNote=state.mapSort==='nearest'?`<div class="distance-disclosure">Distance is straight-line from your current location (“as the crow flies”). Driving distance and ETA may differ.</div>`:'';
 const routeBar=state.routeMode?`<div class="route-bar"><div><b>${state.routeSelected.length} selected</b><small>Tap stores to add/remove route stops.</small></div><div class="route-bar-actions"><button class="chip" data-action="route-clear">Clear</button><button class="chip" data-action="route-cancel">Cancel</button><button class="btn" data-action="route-build" ${state.routeSelected.length?'':'disabled'}>Route selected</button></div></div>`:'';
 return `<section class="section"><div class="section-head"><div><h2>Store map</h2><div class="tiny">${state.routeMode?'Select stores for a navigation route.':'Tap a marker for 7-day activity + quick report.'}</div></div>${isMember()?`<button class="pill" data-action="nav" data-view="products">Manage</button>`:''}</div><div class="map-tools"><button class="chip ${state.mapSort==='recent'?'active':''}" data-action="map-sort" data-sort="recent">Recent</button><button class="chip ${state.mapSort==='nearest'?'active':''}" data-action="map-nearest">Nearest</button><button class="chip ${state.routeMode?'active':''}" data-action="route-mode">${state.routeMode?'Selecting route':'Select route'}</button></div>${locNote}<div class="map-wrap"><div id="map"></div><div class="legend"><b>Legend</b><span><i class="legend-dot green"></i>Stock</span><span><i class="legend-dot red"></i>Empty</span><span><i class="legend-dot gray"></i>Not checked</span><span>👥 Line</span></div></div>${routeBar}</section><section class="section"><div class="section-head"><h2>${state.mapSort==='nearest'?'Nearest stores':'Recently updated'}</h2><button class="pill" data-action="nav" data-view="activity">7-day activity</button></div>${stores.map(store=>{const r=latestReport(store.id),st=mapStatus(store.id),dist=storeDistance(store),selected=selectedRouteStore(store.id);const detail=r?`${r.status==='unsure'?'Unclear':statusMeta[r.status].label} · ${r.period} · ${fmtTime(r.occurredAt)} ${mapFlags(r)}`:'Not checked';return `<button class="store-list-card ${selected?'route-selected':''}" data-action="${state.routeMode?'route-toggle-store':'store-quick'}" data-id="${store.id}"><span class="legend-dot ${st.cls}"></span><span><b>${esc(store.name)}</b><small>${detail}${dist!=null?` · ${dist.toFixed(dist<10?1:0)} mi straight-line`:''}</small></span><span>${state.routeMode?(selected?'✓':'＋'):'›'}</span></button>`}).join('')||'<div class="empty">No active stores.</div>'}</section>`}
function renderMap(){
 if(state.view!=='home'||!document.getElementById('map'))return;
 if(!window.L){document.getElementById('map').innerHTML='<div class="map-fallback">Map library could not load. Store quick views below still work.</div>';return}
 const stores=activeStores(),valid=stores.filter(s=>Number.isFinite(Number(s.lat))&&Number.isFinite(Number(s.lng)));
 const center=state.userLocation?[state.userLocation.lat,state.userLocation.lng]:valid.length?[valid.reduce((a,s)=>a+Number(s.lat),0)/valid.length,valid.reduce((a,s)=>a+Number(s.lng),0)/valid.length]:[42.645,-71.315];
 const map=L.map('map',{zoomControl:false}).setView(center,12);
 L.control.zoom({position:'bottomright'}).addTo(map);
 L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);
 if(state.userLocation){L.circleMarker([state.userLocation.lat,state.userLocation.lng],{radius:7,weight:3,fillOpacity:.85}).addTo(map).bindTooltip('Your current location')}
 valid.forEach(s=>{const st=mapStatus(s.id),r=latestReport(s.id),selected=selectedRouteStore(s.id);const marker=L.marker([s.lat,s.lng],{icon:L.divIcon({className:'tcg-marker-wrap',html:`<button class="tcg-marker ${st.cls} ${selected?'route-selected-marker':''}" aria-label="${esc(s.name)}">${mapFlags(r)}${selected?'<span class="route-marker-check">✓</span>':''}</button>`,iconSize:[38,38],iconAnchor:[19,19]})}).addTo(map);marker.on('click',()=>{if(state.routeMode){toggleRouteStore(s.id);render()}else{state.selectedStore=s.id;state.sheet='store';render()}})});
 if(valid.length>1&&!state.userLocation){const bounds=L.latLngBounds(valid.map(s=>[s.lat,s.lng]));map.fitBounds(bounds.pad(.25),{maxZoom:13})}
}
function toggleRouteStore(id){state.routeSelected=selectedRouteStore(id)?state.routeSelected.filter(x=>x!==id):[...state.routeSelected,id]}
function requestCurrentLocation(){
 return new Promise((resolve,reject)=>{if(!navigator.geolocation)return reject(new Error('Current location is not supported on this device.'));navigator.geolocation.getCurrentPosition(pos=>resolve({lat:pos.coords.latitude,lng:pos.coords.longitude,accuracy:pos.coords.accuracy}),err=>reject(new Error(err.code===1?'Location permission was denied. You can still sort by Recent and use navigation routes.':'Could not get your current location.')), {enableHighAccuracy:false,timeout:12000,maximumAge:300000})})
}
function googleRouteUrl(stores){
 const pts=stores.filter(s=>Number.isFinite(Number(s.lat))&&Number.isFinite(Number(s.lng)));if(!pts.length)return'';
 const enc=s=>`${Number(s.lat)},${Number(s.lng)}`,destination=pts[pts.length-1],waypoints=pts.slice(0,-1).map(enc).join('|');
 const q=new URLSearchParams({api:'1',travelmode:'driving',destination:enc(destination)});
 if(state.userLocation)q.set('origin',`${state.userLocation.lat},${state.userLocation.lng}`);
 if(waypoints)q.set('waypoints',waypoints);
 return `https://www.google.com/maps/dir/?${q.toString()}`;
}
function wazeRouteUrl(store){if(!store)return'';return `https://www.waze.com/ul?ll=${encodeURIComponent(`${store.lat},${store.lng}`)}&navigate=yes`}
...
function products(){
 ...
 const storeSection=member?`<div class="manage-section manage-stores"><div class="manage-section-head"><div><span class="manage-kicker">STORES</span><h3>Add & manage stores</h3><div class="tiny">Search for a location, verify it, then save.</div></div></div><div class="card manage-card">${geoapifyReady()?`<div class="field store-lookup"><label>Find store or address</label><input id="store-search" autocomplete="off" placeholder="Try Target Lowell MA"><div id="store-suggestions" class="lookup-results" hidden></div><div id="store-selection-status" class="tiny lookup-status"></div></div>`:`<div class="lookup-setup">Automatic lookup needs your Geoapify key. Manual entry still works.</div>`}<div class="field"><label>Name</label><input id="new-store" placeholder="Store name"></div><div class="field"><label>Address</label><input id="new-address" placeholder="Street, city, state"></div><div class="two-col"><div class="field"><label>Latitude</label><input id="new-lat" inputmode="decimal" placeholder="Auto-filled"></div><div class="field"><label>Longitude</label><input id="new-lng" inputmode="decimal" placeholder="Auto-filled"></div></div><div id="store-preview-wrap" class="store-preview-wrap" hidden><div class="tiny preview-label">Location preview</div><div id="store-preview-map"></div></div><button class="btn wide" data-action="add-store">Add store</button></div><div class="card manage-list-card">${data.stores.map(s=>`<div class="trend-row"><div><b>${esc(s.name)}</b><div class="tiny">${esc(s.address||'No address')} · ${s.active?'Active':'Archived'}</div></div><button class="chip" data-action="toggle-store" data-id="${s.id}">${s.active?'Archive':'Restore'}</button></div>`).join('')||'<div class="empty">No stores yet.</div>'}</div></div>`:'';
 const productSection=`<div class="manage-divider" role="separator"></div><div class="manage-section manage-products"><div class="manage-section-head"><div><span class="manage-kicker">PRODUCTS</span><h3>Tracked products</h3><div class="tiny">${member?'Add products and review current observed pricing.':'Browse tracked products.'}</div></div>${member?`<button class="pill" data-action="open-product-add">＋ Product</button>`:''}</div><div class="card sku-card"><div class="field"><label>SKU / UPC web lookup</label><div class="inline-input"><input id="sku-lookup" inputmode="numeric" placeholder="Enter SKU or UPC"><button class="btn" data-action="sku-lookup">Search web</button></div><div class="tiny">Opens a web search in a new tab. No paid product API is used.</div></div></div>${activeProducts().map(p=>{const reps=data.reports.filter(r=>r.productId===p.id&&r.price!=null).sort((a,b)=>new Date(b.occurredAt)-new Date(a.occurredAt));const latest=reps[0];return `<button class="product-card" data-action="product-detail" data-id="${p.id}"><div><b>${esc(p.name)}</b><small>${esc(p.tcg)}${p.sku?` · SKU ${esc(p.sku)}`:''}</small></div><div class="product-price">${latest?`$${Number(latest.price).toFixed(2)}<small>${esc(latest.condition||'Observed')}</small>`:'No price'}</div></button>`}).join('')||'<div class="empty">No active products.</div>'}</div>`;
 return `<section class="section"><div class="section-head"><div><h2>Manage</h2><div class="tiny">${member?'Stores and products are separated for faster mobile scanning.':'Tracked products.'}</div></div></div>${storeSection}${productSection}</section>`}
...
function admin(){
 const m=currentMember();if(m?.role!=='admin')return'<div class="empty">Admin only.</div>';
 const levels=(data.settings.rankingTitles||[]).slice().sort((a,b)=>a.min-b.min);
 return `<section class="section"><div class="section-head"><div><h2>Admin</h2><div class="tiny">Full shared-data oversight and corrections</div></div><button class="pill" data-action="members-manage">Manage members</button></div><div class="card"><div class="metric-grid"><div class="metric"><b>${data.members.length}</b><span>Members</span></div><div class="metric"><b>${data.reports.length}</b><span>Reports</span></div><div class="metric"><b>${data.stores.length}</b><span>Stores</span></div></div></div><div class="card admin-data-card"><div class="section-head compact"><div><h3>Data management</h3><div class="tiny">Review, correct or override submitted data in Supabase.</div></div></div><div class="admin-data-actions"><button class="btn" data-action="admin-data" data-kind="reports">Reports</button><button class="btn secondary" data-action="admin-data" data-kind="stores">Stores</button><button class="btn secondary" data-action="admin-data" data-kind="products">Products</button><button class="btn secondary" data-action="export-excel">Export Excel</button></div><div class="tiny" style="margin-top:10px">Excel export includes Reports, Stores, Products and Members as separate worksheets.</div></div><div class="card"><h3>Contribution titles</h3>${levels.map((x,i)=>`<div class="rank-edit"><div class="field"><label>Minimum points</label><input id="rank-min-${i}" inputmode="numeric" value="${Number(x.min)}"></div><div class="field"><label>Title</label><input id="rank-title-${i}" value="${esc(x.title)}"></div></div>`).join('')}<button class="btn wide" data-action="save-ranks">Save ranking titles</button></div><div class="card"><h3>Member activity</h3>${data.members.map(x=>`<div class="member-activity"><div><b>${esc(x.name)}</b><small>${esc(x.email)} · ${esc(rankTitle(x))} · last active ${fmtDate(x.lastActive)}</small></div><div><b>${x.activeDays||0}</b><small>days</small></div><div><b>${x.sessions||0}</b><small>sessions</small></div><div><b>${x.reports||0}</b><small>reports</small></div></div>`).join('')}</div></section>`}

function sheet(){if(state.sheet==='store')return storeSheet(state.selectedStore);if(state.sheet==='report')return reportSheet(state.selectedStore);if(state.sheet==='product-add')return productAddSheet();if(state.sheet==='product-detail')return productDetailSheet(state.productId);if(state.sheet==='report-detail')return reportDetailSheet(state.reportId);if(state.sheet==='members-manage')return membersSheet();if(state.sheet==='profile')return profileSheet();if(state.sheet==='admin-data')return adminDataSheet();if(state.sheet==='admin-edit-report')return adminReportEditSheet(state.adminEditId);if(state.sheet==='admin-edit-store')return adminStoreEditSheet(state.adminEditId);if(state.sheet==='admin-edit-product')return adminProductEditSheet(state.adminEditId);if(state.sheet==='route')return routeSheet();return''}
...
function adminDataSheet(){
 if(currentMember()?.role!=='admin')return'';
 const kind=state.adminDataType||'reports';
 const tabs=`<div class="chips"><button class="chip ${kind==='reports'?'active':''}" data-action="admin-data-kind" data-kind="reports">Reports (${data.reports.length})</button><button class="chip ${kind==='stores'?'active':''}" data-action="admin-data-kind" data-kind="stores">Stores (${data.stores.length})</button><button class="chip ${kind==='products'?'active':''}" data-action="admin-data-kind" data-kind="products">Products (${data.products.length})</button></div>`;
 let rows='';
 if(kind==='reports')rows=data.reports.map(r=>{const s=data.stores.find(x=>x.id===r.storeId),p=data.products.find(x=>x.id===r.productId),member=data.members.find(x=>x.id===r.memberId);return `<div class="admin-record"><div><b>${esc(s?.name||'Unknown store')} · ${esc(statusMeta[r.status]?.label||r.status)}</b><small>${esc(p?.name||'No product')} · ${esc(member?.name||'Unknown member')} · event ${fmtDate(r.occurredAt)} ${fmtTime(r.occurredAt)} · submitted ${fmtDate(r.createdAt)} ${fmtTime(r.createdAt)}</small><small>${esc(r.source||'')} ${r.price!=null?`· $${Number(r.price).toFixed(2)}`:''} ${reportFlags(r)}</small></div><button class="chip" data-action="admin-edit" data-kind="report" data-id="${r.id}">Edit</button></div>`}).join('');
 if(kind==='stores')rows=data.stores.map(s=>`<div class="admin-record"><div><b>${esc(s.name)}</b><small>${esc(s.address||'No address')} · ${s.lat??'—'}, ${s.lng??'—'} · ${s.active?'Active':'Archived'}</small></div><button class="chip" data-action="admin-edit" data-kind="store" data-id="${s.id}">Edit</button></div>`).join('');
 if(kind==='products')rows=data.products.map(p=>`<div class="admin-record"><div><b>${esc(p.name)}</b><small>${esc(p.tcg)} · SKU ${esc(p.sku||'—')} · ${p.active?'Active':'Archived'}</small></div><button class="chip" data-action="admin-edit" data-kind="product" data-id="${p.id}">Edit</button></div>`).join('');
 return `<div class="sheet"><div class="sheet-card"><div class="sheet-title"><div><h2>Admin data</h2><div class="tiny">Changes here override the live Supabase record.</div></div><button class="close" data-action="close-sheet">×</button></div>${tabs}<button class="btn secondary wide export-sheet-btn" data-action="export-excel">Export all to Excel</button><div class="admin-record-list">${rows||'<div class="empty">No records.</div>'}</div></div></div>`;
}
function adminReportEditSheet(id){
 const r=data.reports.find(x=>x.id===id);if(!r)return'';
 return `<div class="sheet"><div class="sheet-card"><div class="sheet-title"><div><h2>Correct report</h2><div class="tiny">Admin override · original submission time is preserved.</div></div><button class="close" data-action="admin-data-back">×</button></div><div class="field"><label>Store</label><select id="ae-store">${data.stores.map(s=>`<option value="${s.id}" ${r.storeId===s.id?'selected':''}>${esc(s.name)}</option>`).join('')}</select></div><div class="field"><label>Product</label><select id="ae-product"><option value="">None</option>${data.products.map(p=>`<option value="${p.id}" ${r.productId===p.id?'selected':''}>${esc(p.tcg)} — ${esc(p.name)}</option>`).join('')}</select></div><div class="two-col"><div class="field"><label>Status</label><select id="ae-status">${['stock','empty','unsure'].map(x=>`<option value="${x}" ${r.status===x?'selected':''}>${statusMeta[x].label}</option>`).join('')}</select></div><div class="field"><label>Time bucket</label><select id="ae-period">${data.settings.periods.map(x=>`<option ${r.period===x?'selected':''}>${x}</option>`).join('')}</select></div></div><div class="field"><label>Indicators</label><div class="check-list"><label><input id="ae-line" type="checkbox" ${r.flags?.line?'checked':''}> 👥 People lining up</label><label><input id="ae-possible" type="checkbox" ${r.flags?.possible?'checked':''}> 🟡 Possible restock</label><label><input id="ae-evidence" type="checkbox" ${r.flags?.evidence?'checked':''}> 🟠 Restock evidence</label></div></div><div class="two-col"><div class="field"><label>Price</label><input id="ae-price" inputmode="decimal" value="${r.price??''}"></div><div class="field"><label>Condition</label><select id="ae-condition"><option value="">—</option>${['Sealed','Unsealed','Damaged'].map(x=>`<option ${r.condition===x?'selected':''}>${x}</option>`).join('')}</select></div></div><div class="field"><label>Source</label><select id="ae-source">${['Firsthand','Friend','Phone call','Social media','Store employee','Other'].map(x=>`<option ${r.source===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>Event time</label><input id="ae-occurred" type="datetime-local" value="${toLocalInput(r.occurredAt)}"></div><label class="admin-check"><input id="ae-approx" type="checkbox" ${r.occurredApprox?'checked':''}> Event time is approximate</label><div class="field"><label>Notes</label><textarea id="ae-notes">${esc(r.notes||'')}</textarea></div><div class="admin-original">Submitted: ${esc(r.createdAt||'—')} · Record ID: ${esc(r.id)}</div><button class="btn wide" data-action="admin-save-report" data-id="${r.id}">Save override</button></div></div>`;
}
function adminStoreEditSheet(id){
 const s=data.stores.find(x=>x.id===id);if(!s)return'';
 return `<div class="sheet"><div class="sheet-card"><div class="sheet-title"><div><h2>Correct store</h2><div class="tiny">Admin override</div></div><button class="close" data-action="admin-data-back">×</button></div><div class="field"><label>Name</label><input id="ase-name" value="${esc(s.name)}"></div><div class="field"><label>Chain</label><input id="ase-chain" value="${esc(s.chain||'')}"></div><div class="field"><label>Address</label><input id="ase-address" value="${esc(s.address||'')}"></div><div class="two-col"><div class="field"><label>City</label><input id="ase-city" value="${esc(s.city||'')}"></div><div class="field"><label>State</label><input id="ase-state" value="${esc(s.state||'')}"></div></div><div class="field"><label>ZIP</label><input id="ase-postal" value="${esc(s.postalCode||'')}"></div><div class="two-col"><div class="field"><label>Latitude</label><input id="ase-lat" inputmode="decimal" value="${s.lat??''}"></div><div class="field"><label>Longitude</label><input id="ase-lng" inputmode="decimal" value="${s.lng??''}"></div></div><label class="admin-check"><input id="ase-active" type="checkbox" ${s.active?'checked':''}> Active</label><button class="btn wide" data-action="admin-save-store" data-id="${s.id}">Save override</button></div></div>`;
}
function adminProductEditSheet(id){
 const p=data.products.find(x=>x.id===id);if(!p)return'';
 return `<div class="sheet"><div class="sheet-card"><div class="sheet-title"><div><h2>Correct product</h2><div class="tiny">Admin override</div></div><button class="close" data-action="admin-data-back">×</button></div><div class="field"><label>Name</label><input id="ape-name" value="${esc(p.name)}"></div><div class="field"><label>TCG</label><input id="ape-tcg" value="${esc(p.tcg)}"></div><div class="field"><label>Set</label><input id="ape-set" value="${esc(p.setName||'')}"></div><div class="two-col"><div class="field"><label>SKU</label><input id="ape-sku" value="${esc(p.sku||'')}"></div><div class="field"><label>UPC</label><input id="ape-upc" value="${esc(p.upc||'')}"></div></div><label class="admin-check"><input id="ape-active" type="checkbox" ${p.active?'checked':''}> Active</label><button class="btn wide" data-action="admin-save-product" data-id="${p.id}">Save override</button></div></div>`;
}
...
async function exportAdminExcel(){
 ...
 const reports=data.reports.map(r=>({
  ...
  People_Lining_Up:!!r.flags?.line,Possible_Restock:!!r.flags?.possible,Restock_Evidence:!!r.flags?.evidence,
 ...
 for(const [name,rows] of [['Reports',reports],['Stores',stores],['Products',products],['Members',members]]){const ws=XLSX.utils.json_to_sheet(rows);XLSX.utils.book_append_sheet(wb,ws,name)}
...
function render(){
 ...
 if(currentMember()||isViewer()){
  ...
  if(state.view==='products'){const lat=document.getElementById('new-lat')?.value,lng=document.getElementById('new-lng')?.value;if(lat&&lng)setTimeout(()=>renderStorePreview(lat,lng),0)}
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(a==='map-sort'){state.mapSort=el.dataset.sort||'recent';render();return}
  if(a==='map-nearest'){
  ...
  if(a==='admin-data'){if(currentMember()?.role!=='admin')return toast('Admin only');state.adminDataType=el.dataset.kind||'reports';state.sheet='admin-data';render();return}
  if(a==='admin-data-kind'){if(currentMember()?.role!=='admin')return;state.adminDataType=el.dataset.kind||'reports';render();return}
  if(a==='admin-data-back'){state.sheet='admin-data';render();return}
  if(a==='admin-edit'){
   if(currentMember()?.role!=='admin')return toast('Admin only');state.adminEditId=el.dataset.id;
   state.sheet=el.dataset.kind==='report'?'admin-edit-report':el.dataset.kind==='store'?'admin-edit-store':'admin-edit-product';render();return;
  }
  if(a==='export-excel'){if(currentMember()?.role!=='admin')return toast('Admin only');toast('Preparing Excel export…');await exportAdminExcel();toast('Excel export created');return}
  if(a==='admin-save-report'){
   if(currentMember()?.role!=='admin')return toast('Admin only');
   const price=parseFloat(document.getElementById('ae-price').value),occurred=document.getElementById('ae-occurred').value;
   await adminUpdateReport(el.dataset.id,{storeId:document.getElementById('ae-store').value,productId:document.getElementById('ae-product').value||null,status:document.getElementById('ae-status').value,period:document.getElementById('ae-period').value,flags:{line:document.getElementById('ae-line').checked,possible:document.getElementById('ae-possible').checked,evidence:document.getElementById('ae-evidence').checked},source:document.getElementById('ae-source').value,notes:document.getElementById('ae-notes').value,price:Number.isFinite(price)?price:null,condition:document.getElementById('ae-condition').value||null,occurredAt:occurred?new Date(occurred).toISOString():iso(),occurredApprox:document.getElementById('ae-approx').checked});
   await refreshShared();state.sheet='admin-data';toast('Report override saved');return;
  }
  if(a==='admin-save-store'){
   if(currentMember()?.role!=='admin')return toast('Admin only');
   const lat=parseFloat(document.getElementById('ase-lat').value),lng=parseFloat(document.getElementById('ase-lng').value);
   if(!Number.isFinite(lat)||!Number.isFinite(lng))return toast('Latitude and longitude are required');
   await adminUpdateStore(el.dataset.id,{name:document.getElementById('ase-name').value.trim(),chain:document.getElementById('ase-chain').value.trim(),address:document.getElementById('ase-address').value.trim(),city:document.getElementById('ase-city').value.trim(),state:document.getElementById('ase-state').value.trim(),postalCode:document.getElementById('ase-postal').value.trim(),lat,lng,active:document.getElementById('ase-active').checked});
   await refreshShared();state.sheet='admin-data';toast('Store override saved');return;
  }
  if(a==='admin-save-product'){
   if(currentMember()?.role!=='admin')return toast('Admin only');
   await adminUpdateProduct(el.dataset.id,{name:document.getElementById('ape-name').value.trim(),tcg:document.getElementById('ape-tcg').value.trim(),setName:document.getElementById('ape-set').value.trim(),sku:document.getElementById('ape-sku').value.trim(),upc:document.getElementById('ape-upc').value.trim(),active:document.getElementById('ape-active').checked});
   await refreshShared();state.sheet='admin-data';toast('Product override saved');return;
  }
  ...
  if(a==='add-store'){
   ...
   if(n&&m&&Number.isFinite(lat)&&Number.isFinite(lng)){await createStore({name:n,address,lat,lng},m.id);state.view='products';state.sheet=null;await refreshShared();toast('Store added')}else toast('Add name, latitude and longitude');return;
  }
  if(a==='save-analytics'){
