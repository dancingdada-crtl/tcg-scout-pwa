...
function activity(){
 ...
 return `<section class="section"><div class="section-head"><div><h2>Activity</h2><div class="tiny">Store history and observed signals.</div></div></div><div class="card"><div class="field"><label>Store</label><select id="activity-store" data-change="activity-store">${stores.map(x=>`<option value="${x.id}" ${storeId===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div><div class="range-row">${[7,30,60,90].map(n=>`<button class="chip ${range===n?'active':''}" data-action="activity-range" data-range="${n}">${n}D</button>`).join('')}</div><div class="store-summary"><b>${esc(store?.name||'Store')}</b><small>${shown.length} reports in ${range} days · Last checked: ${last?fmtDate(last.occurredAt):'Never'}</small></div>${activityMatrix(storeId,range)}<div class="row"><button class="btn" data-action="quick-report" data-id="${storeId}">Quick Report</button><button class="btn secondary" data-action="map-focus-store" data-id="${storeId}">View on Map</button><button class="btn secondary" data-action="drive-store" data-id="${storeId}">Directions</button></div></div>${lastIntel}<div class="section-head"><h2>Report History</h2><span class="tiny">${range}-day range</span></div>${shown.slice(0,100).map(activityRow).join('')||'<div class="empty">No reports in this range.</div>'}</section>`}
